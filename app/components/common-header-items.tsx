...
export function getCoordinatesInfo(
  ...
  if (processed.coordinateSystem) {
    ...
    const coordinateSystemEpsg = processed.coordinateSystem?.epsg ?? "";

    items.push({
      label: t("coordinateSystem"),
      value: [coordinateSystemName, coordinateSystemEpsg]
        .filter((value) => value)
        .join(" "),
    });
  ...
  if (processed.heightSystem) {
    const heightSystemName = processed.heightSystem?.name ?? "";

    items.push({
      label: t("heightSystem"),
      value: heightSystemName,
    });
