...
export function CompactBoreHeader({
  filename,
  data,
  exportMode = false,
}: CompactBoreHeaderProps) {
  const { processed } = data;
  ...
  return (
    <HeaderContainer>
      ...
      <CompactHeaderRightColumn processed={processed} exportMode={exportMode}>
...
export function getBoreHeaderSections(
  data: GefBoreData,
  t: TFunction,
  locale: string,
): Array<HeaderSection> {
...
export function DetailedBoreHeaders({ data }: DetailedBoreHeadersProps) {
  ...
  const allSections = getBoreHeaderSections(data, t, i18n.language);
...
export function StaticDetailedBoreHeaders({ data }: DetailedBoreHeadersProps) {
  const { t, i18n } = useTranslation();
  const allSections = getBoreHeaderSections(data, t, i18n.language);
  ...
  return (
    <div className="space-y-2">
      ...
      <HeaderStaticPanels sections={allSections} />
    </div>
  );
}
