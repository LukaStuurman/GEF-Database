...
import { BoreStyleEditor } from "./bore-style-editor";
...
interface BorePlotProps {
  ...
  onBoreStyleChange?: (style: BoreStyleSettings) => void;
  onBoreStyleReset?: () => void;
  exportMode?: boolean;
}
...
export function BorePlot({
  ...
  height = 800,
  baseFilename,
  boreStyle = createDefaultBoreStyle(),
  onBoreStyleChange,
  onBoreStyleReset,
  exportMode = false,
}: BorePlotProps) {
    ...
    const plot = Plot.plot({
      ...
      marks: [
        ...
        Plot.rect(layers, {
          ...
          y2: "depthBottom",
          fill: (d: BoreLayer) => {
            return getBoreLayerColor(d.soilCode, boreStyle);
          },
          stroke: "white",
          strokeWidth: 0.5,
  ...
  }, [boreStyle, height, i18n.language, layers, specimens, t, width]);
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {legendItems.map((item) => (
            <LegendItem key={item.code} color={item.color} label={item.label} />
          ))}
        </div>
      </div>

      {!exportMode && onBoreStyleChange && onBoreStyleReset ? (
        <BoreStyleEditor
          boreStyle={boreStyle}
          onChange={onBoreStyleChange}
          onReset={onBoreStyleReset}
        />
      ) : null}
