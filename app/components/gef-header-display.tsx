...
export function CompactHeaderRightColumn({
  ...
  const elevationDisplay = elevationValue
    ...
    : null;
  const coordinateSystemName = processed.coordinateSystem?.name ?? "";
  const coordinateSystemEpsg = processed.coordinateSystem?.epsg ?? null;
    ...
    >
      ...
      {processed.coordinateSystem && (
        <>
          ...
          <dd>
            <div>
              <span className="font-semibold">{coordinateSystemName}</span>
...
interface HeaderStaticPanelsProps {
  sections: Array<HeaderSection>;
}

export function HeaderStaticPanels({ sections }: HeaderStaticPanelsProps) {
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-4 items-start">
      {sections.map((section) => (
        ...
        >
          <div className="w-full px-4 py-3 bg-gray-100 text-left">
            <span className="font-medium text-gray-800">{section.title}</span>
          </div>
          ...
          <dl className="space-y-2 p-4 overflow-x-auto">
            {section.items.map((item, index) => (
              <div
                key={index}
                className="grid grid-cols-[160px_1fr] gap-4 text-sm"
              >
                <dt className="text-gray-600">{item.label}</dt>
                <dd className="text-gray-900 font-mono text-xs break-words">
                  {item.value}
                </dd>
              </div>
            ))}
