...
function getCptYAxisDomain(values: Array<number>, reverseY: boolean): [number, number] {
  ...
  if (!reverseY) {
    const domainMin = Math.min(...finiteValues);
    const domainMax = Math.max(...finiteValues);
...
function getCptNapRangeLabel(
  ...
  const startNap = Math.max(...yValues);
  const endNap = Math.min(...yValues);
...
export function CptPlots({
  ...
  const napRangeLabel = currentYAxis.key === "elevation"
    ? getCptNapRangeLabel(data, currentYAxis)
    : null;
      ...
      <div className="flex justify-center">
        {selectedAxes.map((k) => {
          ...
          return (
            ...
            >
              {napRangeLabel ? (
                <div className="mb-2 text-xs text-gray-600">{napRangeLabel}</div>
              ) : null}
              <CptPlot
                plotId={plotId}
                data={data}
...
function CptPlot({
  ...
  useEffect(() => {
    ...
    const yValues = data
      .map((row) => Number(row[yAxis.key]))
      .filter((value) => Number.isFinite(value));
    const yDomain = getCptYAxisDomain(yValues, reverseY);
