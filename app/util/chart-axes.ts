...
import { cptColumnQuantities } from "../../node_modules/@bedrock-engineer/gef-parser/dist/gef-cpt-spec.js";
...
function pushUniqueAxis(
  axes: Array<ChartColumn>,
  axis: ChartColumn | null,
): void {
  if (!axis || axes.some((existingAxis) => existingAxis.key === axis.key)) {
    return;
  }
...
function getElevationAxis(zid: ZID | undefined): ChartColumn {
  const heightSystemName = zid ? (HEIGHT_SYSTEMS[zid.code]?.name ?? null) : null;
  const heightSystemLabel = heightSystemName
    ? /normaal amsterdams peil/i.test(heightSystemName)
      ? "NAP"
      : heightSystemName
    : null;
  ...
  return {
    key: "elevation",
    unit: heightSystemLabel ? (heightSystemLabel === "NAP" ? "NAP" : `m ${heightSystemLabel}`) : "m",
    name: "Elevation",
  };
}
 ...
 *
 * Default behavior:
 * - Y-axis: Elevation/N.A.P. when available, otherwise penetration length (quantity 1)
...
export function detectCptChartAxes(
  ...
  if (hasElevation) {
    pushUniqueAxis(yAxisOptions, getElevationAxis(zid));
  }

  // Add original penetration length as fallback / manual selection
  pushUniqueAxis(
    yAxisOptions,
    yColumn
      ? {
  ...
  pushUniqueAxis(
    yAxisOptions,
    correctedDepthCol && correctedDepthCol.colNum !== yColumn?.colNum
      ? {
